...
def main():
    parser = argparse.ArgumentParser(
        description="Summarize SigProfilerAssignment CNV48 and SV32 activities into cohort-friendly tables."
    )
    parser.add_argument("--input-root", required=True, help="SigProfiler output root containing CNV48/SV32")
    ...
    write_tsv(
        output_dir / "sigprofiler_sv_cna_activity_long.tsv",
        long_rows,
        ["sample_pair", "context_group", "signature", "activity", "fraction_within_context"],
    )
    write_tsv(
        output_dir / "sigprofiler_sv_cna_top_signatures.tsv",
        top_rows,
        [
            "sample_pair",
            "context_group",
    ...
    write_tsv(
        output_dir / "sigprofiler_sv_cna_cohort_totals.tsv",
        cohort_totals,
